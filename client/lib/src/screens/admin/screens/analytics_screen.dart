...
final analyticsDataProvider = FutureProvider.family<AnalyticsData, AnalyticsDateRange>((ref, dateRange) async {
...
AnalyticsData _generateMockAnalyticsData(AnalyticsDateRange dateRange) {
  ...
  final daysDifference = dateRange.endDate.difference(dateRange.startDate).inDays;
  
  // Generate sales data points
  final salesByDate = <SalesDataPoint>[];
  for (int i = 0; i <= daysDifference; i++) {
    final date = dateRange.startDate.add(Duration(days: i));
    final sales = 500.0 + (random % 1000) + (i * 10); // Some random sales with an upward trend
    final orders = 10 + (random % 20) + (i ~/ 3); // Some random orders with a slight upward trend
  ...
  final uniqueCustomers = (totalOrders * 0.8).round(); // Assume 80% unique customers
  ...
  for (int i = 0; i < categories.length; i++) {
    final categorySales = totalSales * (0.1 + (i * 0.15 + (random % 20) / 100)); // Random distribution
  ...
  final returningCustomers = (uniqueCustomers * 0.3).round(); // 30% returning customers
  ...
  final ordersTrend = (totalOrders - previousPeriodOrders) / previousPeriodOrders;
  final avgOrderTrend = (avgOrderValue - previousPeriodAvgOrder) / previousPeriodAvgOrder;
  final customersTrend = (uniqueCustomers - previousPeriodCustomers) / previousPeriodCustomers;

...
class CategoryItemsList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isSelected = order?.dishes.any((dish) => dish.title == item.name) ?? false;
        
        // Get quantity if already selected
        int currentQuantity = 0;
        if (isSelected) {
          currentQuantity = order!.dishes
              .where((dish) => dish.title == item.name)
              .length;
        }
  ...
  void _handleQuantityChange(WidgetRef ref, CateringItem item, int newQuantity, int currentQuantity) {
    ...
    if (newQuantity > currentQuantity) {
      ...
      for (int i = 0; i < (newQuantity - currentQuantity); i++) {
        orderNotifier.addCateringItem(
          CateringDish(
            ...
            ingredients: item.ingredients.map((toElement) => toElement.name).toList(),
            pricing: item.price,
          ),
        );
      }
      ...
      // Remove items
      final dishIndex = order?.dishes.indexWhere((dish) => dish.title == item.name);

import 'package:firebase_ui_auth/firebase_ui_auth.dart' as firebase_auth_ui;
...
final authProvidersProvider = Provider<
    List<
        firebase_auth_ui
        .AuthProvider<firebase_auth_ui.AuthListener, AuthCredential>>>(
  (ref) => [
    firebase_auth_ui.EmailAuthProvider(),
    // Uncomment these as needed
    // firebase_ui.PhoneAuthProvider(),
    // firebase_ui.GoogleProvider(clientId: GOOGLE_CLIENT_ID),
    // firebase_ui.AppleProvider(),
  ],
);
...
final unifiedAuthServiceProvider = Provider<UnifiedAuthService>((ref) {
  ...
  return UnifiedAuthService(auth: auth, firestore: firestore);
});
...
final currentUserProvider = FutureProvider<User?>((ref) {
  ...
  return Future.value(authService.currentUser);
});
...
/// Provider for all admins
final adminsStreamProvider = StreamProvider<List<AdminUser>>((ref) {
  final authService = ref.watch(unifiedAuthServiceProvider);
  return authService.getAdmins();
});

/// Provider for staff by role
final staffByRoleProvider =
    FutureProvider.family<List<StaffMember>, StaffRole>((ref, role) {
  final authService = ref.watch(unifiedAuthServiceProvider);
  return authService.getStaffByRole(role);
});

/// Provider for current staff member
final currentStaffProvider = StreamProvider<StaffMember?>((ref) {
  final authService = ref.watch(unifiedAuthServiceProvider);
  return authService.getCurrentStaffStream();
});

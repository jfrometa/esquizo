...
import 'package:starter_architecture_flutter_firebase/src/core/reservation/reservation_service.dart';
...
class ExampleDataService {
  ...
  /// Create business configuration
  Future<void> _createBusinessConfig(
      String businessId, String businessType, AdminUser adminUser) async {
    final businessConfig = BusinessConfig(
      ...
      name: businessType == 'restaurant'
          ...
          : businessType == 'hotel'
              ...
              : 'Sample Business',
      type: businessType,
      slug: BusinessConfig.generateSlug(businessType == 'restaurant'
          ? 'Sample Restaurant'
          : businessType == 'hotel'
              ? 'Sample Hotel'
              : 'Sample Business'),
      ...
      coverImageUrl: '',
      description:
          'This is a sample $businessType created for demonstration purposes.',
      contactInfo: {
    ...
    await _firestore
        ...
        .doc(businessId)
        .set(businessConfig.toFirestore());
  ...
  Future<Map<String, String>> _createRestaurantCategories(
  ...
  Future<void> _createRestaurantMenuItems(
      String businessId, Map<String, String> categoryIds) async {
    final items = [
      // Appetizers
      {
        'id': 'bruschetta',
  ...
  Future<void> _createRestaurantReservations(
      String businessId, List<String> tableIds) async {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      final timeSlot =
          '${reservationDate.hour}:${reservationDate.minute == 0 ? '00' : '30'}';
  ...
  /// Create hotel rooms
  Future<List<String>> _createHotelRooms(
      String businessId, Map<String, String> categoryIds) async {
  ...
  Future<Map<String, String>> _createGenericCategories(
      String businessId) async {
    final categories = {
  ...
  Future<void> _createGenericProducts(
      String businessId, Map<String, String> categoryIds) async {
    final items = [
      {
        'id': 'product1',
    ...
    try {
      ...
      if (user != null) {
        ...
        if (usersSnapshot.docs.isNotEmpty) {
          ...
          throw Exception(
              'User not found. Please ensure the user exists in the system.');
        }
      }
    } catch (e) {
  ...
  Future<void> _createRestaurantOrders(
      String businessId, List<String> tableIds) async {
  ...
  Future<void> _createHotelBookings(
      String businessId, List<String> roomIds) async {
    final now = DateTime.now();
    ...
    for (int i = 0; i < 5; i++) {
      ...
      final order = Order(
        ...
        items: orderItems,
        status:
            OrderStatus.confirmed, // Using OrderStatus enum instead of string
        subtotal: subtotal,
        tax: tax,
        total: total,

...
class MealPlanService {
  ...
  Future<void> deleteCategory(String id) async {
    ...
    final plansSnapshot =
        await _mealPlansRef.where('categoryId', isEqualTo: id).limit(1).get();
  ...
  Stream<List<MealPlanItem>> getMealPlanItemsStream() {
    return _mealPlanItemsRef.orderBy('name').snapshots().map((snapshot) {
      return snapshot.docs.map((doc) {
        return MealPlanItem.fromMap({
          'id': doc.id,
          ...doc.data() as Map<String, dynamic>,
  ...
  Future<void> toggleMealPlanItemAvailability(
...
final mealPlansByCategoryProvider =
    StreamProvider.family<List<MealPlan>, String>((ref, categoryId) {
  final mealPlanService = ref.watch(mealPlanServiceProvider);
...
// Categories provider
final mealPlanCategoriesProvider =
    StreamProvider<List<MealPlanCategory>>((ref) {
  final mealPlanService = ref.watch(mealPlanServiceProvider);
  return mealPlanService.getCategoriesStream();
});

// Consumed items provider
final consumedItemsProvider =
    StreamProvider.family<List<ConsumedItem>, String>((ref, mealPlanId) {
  final mealPlanService = ref.watch(mealPlanServiceProvider);
